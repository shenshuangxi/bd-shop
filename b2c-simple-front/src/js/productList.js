...
//define(['zepto', "components", "common", "weui", "touch", "template"], function(zepto, components, common, weui, touch, template) {
			...
			components.getMsg(suburl).done(function(msg) {
				...
				if (res == 1) {                
					...
					if(firstCategoryDataList != undefined && firstCategoryDataList != null && firstCategoryDataList.length != 0){
						...
						//绑定事件						
						for (var x in firstCategoryDataList)  
						{
							var first_id=new Object(); 
							first_id= "first_level_"+firstCategoryDataList[x].categoryId; 
							var temp_categoryId=firstCategoryDataList[x].categoryId; 
							$("#"+first_id).click(function() {
								querySecondCategoryListByFirstCategoryId(this.id.substring(12));
							});
						};		
						//获取要显示点击的分类
						if(showCategoryId==undefined ||showCategoryId==""){
							showCategoryId=firstCategoryDataList[0].categoryId;
						}					
						querySecondCategoryListByFirstCategoryId(showCategoryId);
						getPageFrontByGoodsCategory(showCategoryId);
					}else{
						alert("没有一级分类！");	
					}
				}else{
   ...
   //获取二次分类列表，并展示第一个二次分类的数据
	 function querySecondCategoryListByFirstCategoryId(categoryId){	
			//设置一级分类选中效果
			var id="first_level_"+categoryId;
			$('#'+id).siblings().removeClass('active');
			$('#'+id).addClass('active');
							 
			$("#show_second_category").html("");			
			...
			components.getMsg(suburl).done(function(msg) {
				...
				if (res == 1) {                
					...
					if(secondCategoryDataList != undefined && secondCategoryDataList != null && secondCategoryDataList.length != 0){
						var html = template('show_second_category-tpl', msg);
						$("#show_second_category").html(html);	
						//绑定事件						
						for (var y in secondCategoryDataList)  
						{
							var second_id=new Object(); 
							second_id="second_level_"+secondCategoryDataList[y].categoryId; 
							$("#"+second_id).click(function() {
								getPageFrontByGoodsCategory(this.id.substring(13));
								//goodsList.getMore(true,this.id);
							});							
						};	
						
						
						//默认展示第一个二级分类信息
						//getPageFrontByGoodsCategory(secondCategoryDataList[0].categoryId);
						
						
					}else{
						//$("#show_second_category").html("没有二级分类！");
					}					
				}else{
					alert("获取二级分类失败！");
				}
				getPageFrontByGoodsCategory(categoryId);
			});			
	 }	
	 ...
	 function getPageFrontByGoodsCategory(categoryId){							
			//设置二级分类选中效果
			var id="second_level_"+categoryId;
			$('#'+id).siblings().removeClass('active');
			$('#'+id).addClass('active');
			
			$("#show_goods_list").html("");
			tempcategory=categoryId;
			goodsList.getMore(true,categoryId);
			return ;
			//以下是一次加载全部的老方法，废弃掉			
	...
	//滚动
	 $(window).scroll(function() {
        ...
        if ($(document).scrollTop() + $(window).height() + (50 + 50) >= $(document).height()) {					
            goodsList.getMore(false,tempcategory); //获取数据
			$noRec.hide();
        }
    });
	 ...
	 //初始化方法	
	 var requestCategoryId = components.GetQueryString("id");
	 if(requestCategoryId==undefined || requestCategoryId==""){
		 //直接点击的商品链接
		 getAllNormalFirstLevelCategory();
	 }else{
		 //从九宫格过来的商品分类链接
		 getAllNormalFirstLevelCategory(requestCategoryId)
	 }	   
